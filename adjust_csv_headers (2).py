...
def adjust_headers(csv_path):
    """Adjust CSV headers based on expected headers from config.json."""
    # Determine CSV type based on filename
    csv_name = os.path.basename(csv_path)
    if csv_name not in config['expected_headers']:
...
if __name__ == "__main__":
    # Adjust headers for all CSVs in data_output_path
