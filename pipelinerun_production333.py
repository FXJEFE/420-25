...
def post_step_validate(script_name: str, cfg: dict, reg) -> Dict[str, Any]:
    ...
    if script_name == "train_models.py":
        # versioned writes only — never require OG Documents/models/my_model.pkl
        home = Path(os.environ.get("USERPROFILE") or Path.home())
        runs = home / "Documents" / "models" / "og333_runs"
        vers = sorted(
            list(models.glob("my_model_v*.pkl")) + list(runs.glob("my_model_*.pkl")),
            key=lambda p: p.stat().st_mtime if p.is_file() else 0,
            reverse=True,
        )
        p = vers[0] if vers else (models / "my_model.pkl")
        checks.append({"file": str(p), "exists": p.is_file(), "og_untouched": not str(p).endswith(os.path.join("models", "my_model.pkl"))})
